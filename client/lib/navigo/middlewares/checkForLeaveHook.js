...
export default function checkForLeaveHook(context, done) {
  ...
  Q(
    instance.lastResolved().map(function (oldMatch) {
      return function (_, leaveLoopDone) {
        ...
        var runHook = false;
        var newLocationVSOldMatch = context.instance.matchLocation(
          oldMatch.route.path,
        ...
        if (oldMatch.route.path !== '*') {
          ...
          var someOfTheLastOnesMatch = context.matches
            ? context.matches.find(function (match) {
